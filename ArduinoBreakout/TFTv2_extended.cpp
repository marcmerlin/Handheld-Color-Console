...
- re-ident to compare with breakout version and unify both --merlin
...
void TFT::drawChar(INT8U ascii, INT16U poX, INT16U poY, INT16U size,
		   INT16U fgcolor)
{
	...
	for (int i = 0; i < FONT_X; i++) {
		...
		for (INT8U f = 0; f < 8; f++) {
			if ((temp >> f) & 0x01) {
				fillRectangle(poX + i * size, poY + f * size,
					      size - 1, size - 1, fgcolor);
			}
		}
	}
}
...
void TFT::drawString(char *string, INT16U poX, INT16U poY, INT16U size,
		     INT16U fgcolor)
{
	while (*string) {
...
void TFT::drawCenteredString(char *string, INT16U poy, INT16U size,
			     INT16U fgcolor)
{
...
void TFT::drawStringWithShadow(char *string, INT16U poX, INT16U poY,
...
void TFT::fillRectangleUseBevel(INT16U x, INT16U y, INT16U w, INT16U h,
				INT16U color)
{
	INT16U x2 = x + w;
	INT16U y2 = y + h;
...
void TFT::drawRectangle(INT16U poX, INT16U poY, INT16U length, INT16U width,
			INT16U color)
{
...
void TFT::drawRectangle(INT16U poX, INT16U poY, INT16U length, INT16U width,
			INT16U color, byte thickness)
{
	for (int i = 0; i < thickness - 1; i++) {
		drawHorizontalLine(poX + i, poY + i, length - i * 2, color);
		drawHorizontalLine(poX + i, poY + width - i, length - i * 2,
				   color);
		drawVerticalLine(poX + i, poY + i, width - i * 2, color);
		drawVerticalLine(poX + length - i, poY + i, width - i * 2,
				 color);
	}
}
...
void TFT::drawTriangle(int poX1, int poY1, int poX2, int poY2, int poX3,
		       int poY3, INT16U color)
{
...
INT8U TFT::drawNumber(long long_num, INT16U poX, INT16U poY, INT16U size,
		      INT16U fgcolor)
{
...
INT8U TFT::drawFloat(float floatNumber, INT8U decimal, INT16U poX, INT16U poY,
...
INT8U TFT::drawFloat(float floatNumber, INT16U poX, INT16U poY, INT16U size,
		     INT16U fgcolor)
{
	INT8U decimal = 2;
	INT16U temp = 0;
